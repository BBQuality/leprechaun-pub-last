...
class WineFranceSeeder extends Seeder
{
    public function run(): void
    {
        $categoryId = DB::table('menu_categories')->where('key', 'wine_france')->value('id');

        if (!$categoryId) {
            throw new \Exception('Категорія "wine_chile" не знайдена.');
        }

        $items = [
            [
                'title_ua' => 'Вино Франції',
                'title_en' => 'Wine France',
